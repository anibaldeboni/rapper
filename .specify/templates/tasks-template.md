...
**Tests**: Include verification tasks for every behavior change. At minimum, cover affected
package tests and any integration/contract checks required by the feature.
...
- [ ] T004 Define Constitution Check evidence (architecture, config contract, verification,
      operational safety)
...
> **NOTE: Define and run required tests before declaring story completion**
...
- Required verification tasks MUST complete before implementation sign-off
...
- Include `make test` and `make lint` execution tasks before completion
